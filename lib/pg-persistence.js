...
module.exports = class PgPersistence {
  ...
  async createTodoList(title) {
    let CREATE_TODOLIST = `INSERT INTO todolists (title, username) VALUES ($1, $2)`;
  ...
  async deleteTodo(todoListId, todoId) {
    const DELETE_TODO = `DELETE FROM todos 
                         WHERE id = $1 AND todolist_id = $2 AND
                         username = $3`;

    let result = await dbQuery(DELETE_TODO, todoId, todoListId, this.username);
    return result.rowCount > 0;
  }
